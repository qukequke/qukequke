...
def pull_screenshot(my_add):
    ...
    if screenshot_way == 2 or screenshot_way == 1:
        ...
        f = open('./dataset2/'+str(my_add)+'autojump.png', 'wb')
...
def check_screenshot(my_add):
    ...
    if (screenshot_way < 0):
        print('暂不支持当前设备')
        sys.exit()
    pull_screenshot(my_add)
    try:
        Image.open('./autojump.png').load()
        print('采用方式 {} 获取截图'.format(screenshot_way))
    except Exception:
        screenshot_way -= 1
        check_screenshot(my_add)
...
def save_press_time_txt(data):
    f = open('./dataset2/press_time.txt', 'a+')
    f.write(str(data))
    f.write(',')
    f.close()
...
def main():
    ...
    my_add_num = 1177
    # check_screenshot(my_add_num)

    i, next_rest, next_rest_time = 0, random.randrange(30, 40), random.randrange(5, 10)
    press_time_list = []
    while True:`
        pull_screenshot(my_add_num)
        im = Image.open('./dataset2/'+str(my_add_num)+'autojump.png')
        my_add_num += 1
        # 获取棋子和 board 的位置
        ...
        save_press_time_txt(press_time)
        if debug_switch:
